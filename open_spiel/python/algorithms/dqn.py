...
class DQNBot(pyspiel.Bot):
  ...
  def step(self, state):
    ...
    legal_actions = [state.legal_actions(player_id) for _ in range(self._num_players)]
    info_state = [state.observation_tensor(player_id) for _ in range(self._num_players)]
    ...
    action = agent_output.action # TODO expand functionality to simultaneous games with apply_actions()
    return action